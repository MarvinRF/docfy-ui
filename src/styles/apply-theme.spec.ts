...
describe('applyTheme()', () => {
  ...
  it('sets all 5 CSS custom properties', () => {
    ...
    expect(root.style.getPropertyValue('--color-bg')).toBe('#393E46');
    expect(root.style.getPropertyValue('--color-text')).toBe('#FFFAF3');
    expect(root.style.getPropertyValue('--color-accent')).toBe('#E95420');
    expect(root.style.getPropertyValue('--color-bg-elevated')).toMatch(/^#[0-9a-f]{6}$/i);
    expect(root.style.getPropertyValue('--color-border')).toMatch(/^#[0-9a-f]{6}$/i);
  });
